...
- Nova página **Integrations** no `/console` para configurar o "cérebro" LLM.
- Lista providers de `GET /admin/llm-providers`; adiciona/atualiza via
  `POST /admin/llm-providers` (id, kind, base_url, model, api_key write-only,
  flag default). Clicar numa linha carrega-a no formulário.
- Botões **Validate** (local) e **Ping** (ao vivo) sobre
  `POST /admin/llm-providers/{validate,ping}`, mostrando ok/mensagem e latência;
  uma URL ruim retorna ok:false inline sem quebrar.
- Painel de exposição da API de `GET /admin/api-info` (base URL + hint da key
  mascarada) com exemplo curl de recall. A API key armazenada nunca é mostrada.
- Sem mudanças no servidor; assets rebuildados e embarcados.

Anterior: **Control Plane TS — Upload de Arquivos (drag-and-drop) v0.7**:
...
- Nova página **SQL Editor** no `/console`, condicionada a um tenant ativo,
  ...
  records).
- Editor monospace com botão Run e `Ctrl/Cmd + Enter`; snippets de exemplo
  preenchem o editor sem executar automaticamente.
- Resultados de sucesso renderizam como tabela com colunas deterministas e
  ordenadas de payload (id/table como colunas de sistema), um toggle Raw JSON e
  a contagem de linhas e duração observadas no cliente.
- O `ApiError` tipado expõe erros de parse/validação inline sem quebrar.
- Helper compartilhado `format.ts` (colunas de record, cell, truncate) agora
  embasa tanto o Data Explorer quanto o SQL Editor.
- Sem mudanças no servidor; assets rebuildados e embarcados.

Anterior: **Control Plane TS — Data Explorer (leitura) v0.3**:
...
- Novo endpoint `POST /admin/tenants/{tid}/files` multipart (feature `multipart` do axum).
  ...
  - `.pdf` → `store_document(from_pdf(...))` — Document chunked para recall
  - `.txt`, `.md` → `store_document(new(...))` — Document texto puro
  - `.csv` → parser CSV inline → um `put_record()` por linha; tabela = nome do arquivo
  - `.json` — array de objetos → Records; qualquer outro JSON → Document
  - outras extensoes → `422 Unprocessable Entity`
  - Resposta: `{ file_id, kind, count, name }`
- Novo módulo `handlers/files.rs` seguindo o padrão dos handlers existentes.
- Pagina Files agora exibe uma zona drag-and-drop Liquid Glass com badges por tipo de arquivo,
  upload XHR com barra de progresso em tempo real e banner de sucesso apos upload.
- Arrastar arquivos para `#dropZone` ou clicar para navegar ambos disparam o upload.
- `styles.css`: novas classes `.drop-zone`, `.drag-over`, `.drop-zone-progress`,
  `.drop-zone-progress-bar`, `.file-type-badge`, `.upload-result`, `.radio-card`.

Anterior: **Control Plane — Liquid Glass UI v0.1**:

- Redesign visual completo do `/admin` em interface Liquid Glass premium:
  cards translucidos com `backdrop-filter: blur`, fundo com gradiente radial,
  accent cyan com glow, transicoes suaves, drawer com animacao slide-in,
  notificacoes toast e empty states com orientacao de proximos passos.
- Nova pagina `Documentation` com docs bilíngues embutidos (9 secoes:
  Visao Geral, Quickstart, Conceitos, Modelo de Dados, Guia SQL, Guia RAG,
  Autenticacao API, Guia CLI, Arquitetura) e navegacao em sidebar sticky.
- `DashboardPage()` agora exibe welcome card com 4 passos de onboarding
  quando nao ha tenants, guiando novos usuarios por tenant → collection →
  ingestao → recall.
- Empty states em todas as paginas (Tenants, Collections, Records, Memories,
  Documents, Files, Data Explorer) com texto de orientacao e botoes contextuais.
- Grade de quick-action cards no Dashboard com classe CSS `.quick-action-card`
  e indicador de seta.
- Atalhos de navegacao (`go-tenants`, `go-collections`, `go-sql`, `go-explorer`,
  `go-ingest`, `go-keys`, `go-docs`) acessiveis de qualquer empty state.
- `index.html` atualizado para importar a fonte Inter via Google Fonts.
- Toda a funcionalidade existente preservada; nenhuma pagina removida; i18n PT/EN mantido.
- Portao de qualidade: `cargo fmt`, `cargo test --workspace`, `cargo clippy -D warnings` todos verdes.

Anterior: **MMR (Maximal Marginal Relevance) v0.1**:
